...
- 删除任务时可勾选“同时删除下载文件”，默认保留已下载文件；设置页支持清空所有下载任务。
...
当前版本为 `v1.0.16`。从 GitHub Releases 下载最新 APK：
...
在任务菜单中点击「删除」，或进入「设置 → 存储 → 清空所有下载任务」，确认后移除任务记录与缓存。正在执行的任务会先停止。

确认框中的「同时删除下载文件」默认不勾选；勾选后仅删除任务对应的文件，不清空整个保存目录。文件删除失败时保留对应任务并提示原因，其他任务仍可正常删除。
...
- `version_name`：下一个语义化版本号，例如 `1.0.17`
- `version_code`：大于当前 `116` 的 Android `versionCode`
- `prerelease`：是否标记为预发布
